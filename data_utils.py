...
def basic_tokenizer_aq(line, normalize_digits=True,is_question= True):
    """
    A tokenizer to tokenize text into tokens.
    :param line: a string of sentence.
    :param normalize_digits: whether to normalize digits.
...
def build_vocab(filename,is_question = True):
    """
    Build vocabulary.
    ...
    with open(in_path, encoding="utf-8") as f:
        for line in f.readlines():
            for token in basic_tokenizer_aq(line,is_question = is_question):
...
def sentence2id(vocab, line,is_question = True):
    """
    ...
    return [vocab.get(token, vocab["<unk>"]) for token in basic_tokenizer_aq(line,is_question=is_question)]
...
def token2id(data, mode):
    ...
    for line in lines:
        if mode == "dec":  # we only care about <s> and </s> in decoder
            ids = [vocab["<s>"]]
            ids.extend(sentence2id(vocab, line,False))
        else:
            ids = []
...
def process_data():
    ...
    enc_vocab_size = build_vocab("train.enc",is_question=True)
